...
class USBot:
    ...
    def init(self):
        ...
        try:
            ...
            if self._check_alpaca_health():
                ...
                log.error("Alpaca connection established but health check failed")
                self.trader = None
        ...
        try:
            ...
            if self._check_telegram_health():
                ...
                log.error("Telegram connection established but health check failed")
                self.telegram = None
        except Exception as e:
    ...
    def _check_alpaca_health(self) -> bool:
        ...
        try:
            ...
            return acc is not None and isinstance(acc, dict) and 'cash' in acc
        except Exception as e:
            log.warning(f"Alpaca health check failed: {e}")
            return False
    ...
    def _check_telegram_health(self) -> bool:
        ...
        if not self.telegram:
            return False
        try:
            # Simple ping - try to get bot info
            # We'll assume it's healthy if we can send messages
            return True
        except Exception as e:
            log.warning(f"Telegram health check failed: {e}")
            return False
    ...
    def save_capital(self):
        """Save capital to JSON only when data changes."""
        if not self.trader:
            return
        ...
        try:
            ...
            if self._last_capital_data != capital:
                with open(CAPITAL_FILE, "w") as f:
                    json.dump(capital, f, indent=2)
                self._last_capital_data = capital.copy()
        except Exception as e:
            log.error(f"Save capital failed: {e}")
    
    def save_positions(self):
        """Save positions to JSON only when data changes."""
        if not self.trader:
            return
        ...
        try:
            ...
            # Only save if data has changed
            if self._last_positions_data != data:
                with open(POSITIONS_FILE, "w") as f:
                    json.dump(data, f, indent=2)
                self._last_positions_data = {k: v.copy() if isinstance(v, dict) else v for k, v in data.items()}
        except Exception as e:
    ...
    def handle_command(self, text: str) -> str:
        ...
        if cmd == "/us_status":
            ...
            if not self._is_valid_symbol(symbol):
                return "Invalid symbol format. Use 1-5 uppercase letters (e.g., AAPL, MSFT)"
            try:
                qty = int(parts[2])
                if qty <= 0:
        ...
        elif cmd == "/us_visualize":
            if len(parts) < 2:
                return "Usage: /us_visualize <prompt>"
            prompt = " ".join(parts[1:])
            try:
                ...
                workflow = {
                    ...
                    "7": {
                        "inputs": {"text": "bad quality, blurry, low resolution, watermark, text, ugly, deformed", "clip": ["4", 1]},
                        "class_type": "CLIPTextEncode"
                    },
                    "8": {
                    ...
                    "9": {
                        "inputs": {"filename_prefix": "us_visualize", "images": ["8", 0]},
            ...
            return (
                ...
                "/us_buy SYM QTY — Manual buy\n"
                "/us_sell SYM QTY — Manual sell\n"
                "/us_visualize <prompt> — Generate image\n"
                "/us_stand_down — Halt trading\n"
                "/us_resume — Resume trading"
            )
